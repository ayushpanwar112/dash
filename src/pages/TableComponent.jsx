...
const TableComponent = () => {
  ...
  const handleAddEntry = async (e) => {
    ...
    try {
      if (editingId) {
        ...
        await axiosInstance.put(`/api/table/${editingId}`, formData);
        setTableData((prev) => prev.map((item) => (item._id === editingId ? { ...item, ...formData } : item)));
        setEditingId(null);
      } else {
        // Add new entry
        const response = await axiosInstance.post(`/api/table`, formData);
        setTableData((prev) => [...prev, response.data.data]);
      }
      setFormData({ month: "", carriedForward: "", received: "", resolved: "", pending: "" });
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      await axiosInstance.delete(`/api/table/${id}`);
  ...
  return (
    <div className="container mx-auto p-6">
      <h2 className="text-2xl font-bold mb-4">Table Data for Monthly</h2>

      <form onSubmit={handleAddEntry} className="mb-6 grid grid-cols-2 gap-4">
        <input type="text" name="month" placeholder="Month" value={formData.month} onChange={handleChange} required className="border p-2 rounded w-full" />
